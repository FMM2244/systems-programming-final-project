...
static void sendAll(int fd, const char *msg, size_t len) {
	size_t total = 0;
	while (total < len) {
...
static void sendMatrixResult(int client_fd, mtrx_t *mat) {
	...
	buffer[offset] = '\0';
	sendAll(client_fd, buffer, strlen(buffer));
}
...
static void *clientHandler(void *arg) {
	...
	char choice_buf[16];
	ssize_t received = recv(client_fd, choice_buf, sizeof(choice_buf) - 1, 0);
	if (received <= 0) {
		close(client_fd);
		return NULL;
	}
	choice_buf[received] = '\0';
	...
	if (choice == 1) {
		...
		if (multiply(&C) == 0) {
			...
			sendAll(client_fd, "Error: Can't Multiply Matrices\n", 31);
		}
	}
	else if (choice == 2) {
		mtrx_t T;
		if (transposition('A', &T) == 0) {
			...
			sendAll(client_fd, "Error: Can't Transpose Matrix\n", 30);
		}
	}
	else if (choice == 3) {
		...
		snprintf(reply, sizeof(reply), "Invalid option.\n");
		sendAll(client_fd, reply, strlen(reply));
	}
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		*client_fd = newsockfd;
		...
		pthread_t th;
		if (pthread_create(&th, NULL, clientHandler, client_fd) != 0) {
